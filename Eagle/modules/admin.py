...
admins = app.get_chat_members(chat_id, filters=enums.ChatMembersFilter.ADMINISTRATORS)
...
async def unmute(_, message: Message):
    if await CheckAdmin(message) is True:
        ...
        if reply:
            ...
            user = get_arg(message)
            if not user:
                await message.edit("**Whom should I unmute?**")
                return
        try:
            get_user = await app.get_users(user)
            await app.restrict_chat_member(
...
async def kick_user(_, message: Message):
    if await CheckAdmin(message) is True:
        ...
        try:
            ...
            await message.edit(f"**Kicked {get_user.first_name} ass from the chat.**")
        except:
