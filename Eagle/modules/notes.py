...
import Eagle.database.notesdb as eagledb
...
async def save(client, message):
    ...
    if not arg:
        ...
        return
    note_name = arg
    note = await eagledb.get_note(note_name)
    if note:
    ...
    await eagledb.save_note(note_name, copy.message_id)
...
async def clear(client, message):
    ...
    if not note:
        ...
        return
    await eagledb.rm_note(note_name)
...
async def notes(client, message):
    msg = "**Saved Notes**\n\n"
    all_notes = await eagledb.all_notes()
    if not all_notes:
